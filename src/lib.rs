...
fn wright_fisher_sim(stem_cell_size: usize, generation_time: u64) -> (Vec<Vec<i64>>, Vec<u64>) {
    ...
    let time_vector: Vec<u64> = (0..current_gen)
        .map(|x| x as u64 * generation_time)
        .collect();
    (pop, time_vector)
}
...
fn auc(x: &[u64], y: &[usize]) -> f64 {
    ...
    x.windows(2)
        .zip(y.windows(2))
        .map(|(xw, yw)| direction * (xw[1] - xw[0]) as f64 * (yw[0] + yw[1]) as f64 / 2.0)
        .sum()
}
...
#[pymodule]
fn wfrs(m: &Bound<'_, PyModule>) -> PyResult<()> {
